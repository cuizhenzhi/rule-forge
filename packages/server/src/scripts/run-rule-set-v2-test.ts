/**
 * Final test evaluation of frozen rule_set_v2. Run ONCE after selection.
 * Registers experiment in DB.
 */
...
function main(): void {
  ...
  if (!ruleSet.frozen) {
    console.error('rule_set_v2 is not frozen. Run select-rule-set-v2 first.');
    process.exit(1);
  }
  ...
  const metricsPath = join(REPO_ROOT, 'data', 'rule_experiments', 'rule_set_v2_test_metrics.json');
  writeFileSync(metricsPath, JSON.stringify(metricsOut, null, 2), 'utf-8');
